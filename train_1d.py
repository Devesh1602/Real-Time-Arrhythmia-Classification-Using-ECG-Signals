...
def run():
    ...
    for key, pair in encoding.items():
        encoding[key] = integer_encoded[np.where(y_train == key)[0][0]]
    ...
    model = define_model()
    filepath = "model_1d_balanced_class_balanced_validation-{epoch:02d}-{val_acc:.2f}.h5"
    checkpoint = ModelCheckpoint(filepath, monitor='val_acc', verbose=1, save_best_only=True, mode='max')
    callbacks_list = [checkpoint]
    history = model.fit(X_train, y_train, batch_size=1024, epochs=50, validation_data=(X_validation, y_validation), callbacks= callbacks_list)
    ...
    #model.save_weights('model_1d_balanced_class_balanced_validation.h5')
    ...
    with open('Results.txt', 'a+') as f:
        f.write("300 segment with balanced class and balanced split- " + "%s: %.2f%%" % (model.metrics_names[1], eval_model[1] * 100) + "  " + "%s: %.2f%%" % (model.metrics_names[0], eval_model[0]) +
                ' Testing Accuracy - ' + str(acc*100) + ' Sensitivity - '+ str(np.mean(sensitivity)*100) + ' Specificity - '+ str(np.mean(specificity)*100) + ' F1 Score - '+ str(f1_scor*100) +'\n')
    f.close()
